"""Builder-side source interface.

Parallel to the downloader's :class:`gacdi.base.BaseDownloadSource`, but for the
*builder* half. A :class:`BaseManifestSource` owns the source-specific work —
define CLI flags, turn requirements into a native query, count/preview, and fetch
records as :class:`~gacdi_manifest.model.FileRow` objects. The shared runner in
:mod:`gacdi_manifest.cli` owns everything that is the same for every source: the
annotation join, the post-query metadata filter, and writing the manifest /
metadata / report.

Each source lives in ``sources/<name>.py`` and is listed in
:mod:`gacdi_manifest.registry`.

``BuildImporter`` is retained as a compatibility alias (see below).
"""
...
# Compatibility alias: the historical base-class name. ``BaseManifestSource`` is
# the preferred internal name.
BuildImporter = BaseManifestSource
